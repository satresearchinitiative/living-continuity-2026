...
articleFiles.forEach(file => {
  ...
  // Check if already has RotatingImage
  if (content.includes('RotatingImage') && content.includes(imagePath)) {
    return; // Already updated
  }
  
  // Check if still has mesh pattern
  if (content.includes('<mesh') && content.includes('geometry={nodes.')) {
    ...
    // Replace mesh group with RotatingImage - more flexible pattern
    ...
    if (meshPattern.test(content)) {
      ...
      // Try simpler pattern - just find the mesh group
      const simplePattern = /<group>\s*<mesh[\s\S]*?geometry=\{nodes\.[^}]+\}[\s\S]*?<\/mesh>[\s\S]*?<mesh[\s\S]*?geometry=\{nodes\.[^}]+\}[\s\S]*?<\/mesh>[\s\S]*?<\/group>/;
      if (simplePattern.test(content)) {
        ...
        fs.writeFileSync(file, content);
        console.log(`Fixed ${basename} (simple pattern)`);
      }
    }
  }
});
